...
// 支持的所有连接类型（数据库 + Jenkins）
const VALID_TYPES = [...VALID_DB_TYPES, 'jenkins'];
...
class ConfigManager {
  ...
  addConnection(alias, config, options = {}) {
    ...
    const requiredFields = config.type === 'jenkins' ? REQUIRED_JENKINS_FIELDS : REQUIRED_DB_FIELDS;
  ...
  listConnections() {
    ...
    for (const [alias, config] of Object.entries(userConnections)) {
      ...
      result.push({
        alias,
        type: config.type || '',
        host: config.type === 'jenkins' ? (config.baseUrl || '') : (config.host || ''),
        db: config.db || '',
        source: 'user'
      });
    }
